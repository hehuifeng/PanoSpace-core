...
class CacheManager:
    # Bump when the on-disk feature/cache format changes in a backward-incompatible way.
    _CACHE_VERSION = "v2"
...
class FeatureExtractor:
    ...
    def __init__(self, local_path, pretrained_model_name,
    ...
    def extract_all(self, centers, img_path, radius: int, neighb: int,
                    batch_size: int = 64, desc: str = "extract"):
        ...
        for start in tqdm(range(0, n, batch_size), desc=desc):
            ...
            # one ViT forward.
            crops = [self.tf(img.crop((int(x) - r, int(y) - r, int(x) + r, int(y) + r)))
                     for x, y in batch_centers]
            crops += [self.tf(img.crop((int(x) - r * k, int(y) - r * k,
                                        int(x) + r * k, int(y) + r * k)))
                      for x, y in batch_centers]
...
class CachedFeatureDataset(Dataset):
    ...
    def __init__(self, center_feat: torch.Tensor, neighbor_feat: torch.Tensor,
        ...
        if labels is not None and labels.shape[0] != center_feat.shape[0]:
            raise ValueError(
                f"labels length {labels.shape[0]} != features length {center_feat.shape[0]}"
            )
...
class DINOv2NeighborClassifier(pl.LightningModule):
    ...
    def __init__(self, num_classes=9, class_weights=None, learning_rate=1e-4,
                 feature_dropout: float = 0.1, noise_std: float = 0.0,
    ...
    def configure_optimizers(self):
        ...
        sched = torch.optim.lr_scheduler.CosineAnnealingLR(
            opt, T_max=max_epochs, eta_min=self.learning_rate * 0.01,
        )
...
class DINOv2_superres_deconv(object):
    def __init__(self,
        ...
        if os.path.exists(ckpt_path):
            ...
            self.model = DINOv2NeighborClassifier.load_from_checkpoint(
                ckpt_path, num_classes=num_classes, class_weights=class_weights,
            )
        else:
    ...
    def _precompute_features(self, device="cuda",
                             precision: Literal["16-mixed", "32"] = "16-mixed",
                             batch_size: int = 64):
        ...
        if os.path.exists(feat_path):
            ...
            if (meta.get("radius") == self.radius
        ...
        spot_cf, spot_nf = extractor.extract_all(
            spot_centers, self.img_dir, radius=self.radius, neighb=self.neighb,
            batch_size=batch_size, desc="spot-features",
        )
        sr_centers = np.asarray(self.sr_adata.obsm['spatial'])
        sr_cf, sr_nf = extractor.extract_all(
            sr_centers, self.img_dir, radius=self.radius, neighb=self.neighb,
            batch_size=batch_size, desc="subspot-features",
        )
        ...
        bundle = {
            "spot_center": spot_cf, "spot_neighbor": spot_nf, "spot_labels": spot_labels,
            "sr_center": sr_cf, "sr_neighbor": sr_nf,
            "meta": {
                "radius": self.radius, "neighb": self.neighb,
                "num_classes": self.num_classes, "celltypes": self.cell_type_name,
        ...
        logger.info("Saved features to %s (spot=%d, subspot=%d)",
                    feat_path, spot_cf.shape[0], sr_cf.shape[0])
        return bundle
    ...
    def _predict(self, dataloader, device):
        ...
        with torch.inference_mode():
            for cf, nf in tqdm(dataloader, desc="superres-predict"):
                logits = model(cf.to(device, non_blocking=True),
                               nf.to(device, non_blocking=True))
                outs.append(F.softmax(logits, dim=1).cpu())
        return torch.cat(outs, dim=0).numpy()

    def run_train(self, epoch=50, batch_size=32, num_workers=0,
                  accelerator="gpu", precision="16-mixed",
                  devices: Union[int, str] = "auto", seed: int = 42,
        ...
        feat = self._precompute_features(
            device=extract_device, precision=extract_precision,
