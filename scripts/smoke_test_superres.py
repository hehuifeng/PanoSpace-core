...
def _build_fake_dataset(n_spots: int = 20, img_size: int = 600,
                        radius: int = 30, seed: int = 0) -> ad.AnnData:
...
def _run_once(adata: ad.AnnData, img_path: str, cache_dir: str,
              accelerator: str, precision: str, dinov2_path: str,
              epoch: int = 3) -> tuple[float, "ad.AnnData", str]:
...
def _assert_outputs(sr: "ad.AnnData", n_spots: int):
    ...
    assert sr.n_obs > n_spots, (
        f"Expected subspot count > spot count ({n_spots}), got {sr.n_obs}"
    )
    ...
    assert np.allclose(row_sums, 1.0, atol=1e-4), (
        f"Per-subspot probabilities must sum to 1; got range "
...
def main():
    ...
    parser.add_argument("--cpu", action="store_true",
                        help="Force CPU + fp32 (default: GPU + 16-mixed if CUDA available)")
    parser.add_argument("--dinov2", default=os.path.expanduser("~/.panospace_cache/dinov2-base"),
                        help="Local path to DINOv2 weights")
    parser.add_argument("--n-spots", type=int, default=20)
    parser.add_argument("--img-size", type=int, default=600)
    parser.add_argument("--epoch", type=int, default=3)
    parser.add_argument("--cache-dir", default=None,
                        help="Cache dir (default: a fresh tempdir)")
    ...
    if not Path(args.dinov2).exists():
        print(f"ERROR: DINOv2 weights not found at {args.dinov2}. "
              f"Pass --dinov2 /path/to/dinov2-base", file=sys.stderr)
        sys.exit(2)
    ...
    try:
        ...
        adata = _build_fake_dataset(n_spots=args.n_spots,
                                    img_size=args.img_size, radius=30, seed=0)
        print(f"synthetic: {args.n_spots} spots, image {args.img_size}x{args.img_size}")

        # First pass: must extract features + train + predict.
        elapsed1, sr1, _ = _run_once(adata, img_path, cache_dir,
                                     accelerator, precision, args.dinov2,
                                     epoch=args.epoch)
        ...
        from panospace._core.annotation._superres_backend.superres_utils import CacheManager
        cm = CacheManager(base_dir=cache_dir)
        params = {"radius": 30, "neighb": 2, "num_classes": len(CELLTYPES),
                  "celltypes": CELLTYPES, "dinov2_model": "facebook/dinov2-base"}
        cache_path = cm.get_cache_path(img_path, params)
        feat_path = os.path.join(cache_path, "features.pt")
        ...
        print(f"[pass 1] features.pt ({os.path.getsize(feat_path)//1024} KB), "
        ...
        adata2 = _build_fake_dataset(n_spots=args.n_spots,
                                     img_size=args.img_size, radius=30, seed=0)
        elapsed2, sr2, _ = _run_once(adata2, img_path, cache_dir,
                                     accelerator, precision, args.dinov2,
                                     epoch=args.epoch)
        print(f"[pass 2] cached pipeline OK in {elapsed2:.1f}s "
              f"(should be much faster than pass 1)")
        ...
        assert np.allclose(p1, p2, atol=1e-4), (
            f"Cached-run predictions diverged: max diff {np.abs(p1-p2).max()}"
        )
